...
- Confirm route metadata remains complete and the final preview builds without errors.
  ## Important performance and data integrity safeguard
  - Never load or render the full 149,860-record Spotify dataset directly into the React UI.
  - Perform aggregation and analysis outside the rendered component tree and memoize derived results.
  - Use pagination or virtualization for record-level exploration.
  - Keep the initial page fast by loading only the summaries/aggregates needed for the first view.
  - Do not replace real dataset-driven analysis with fabricated demo data merely to make interactions work.
  - If a cross-dataset relationship cannot be established from the available fields, describe it as temporal proximity rather than claiming the records belong to the same person or imply a causal relationship.