...
function PatternsPage() {
  ...
  return (
    <LifeTraceShell>
      <PageIntro index="03" eyebrow="Patterns" title="Things you kept coming back to.">Repeated forms emerge when the archive is viewed from a distance. Every statement here can be traced to a count, time, or recorded field.</PageIntro>
        ...
        <div className="grid gap-12 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="border-t border-border">
            {patterns.map((pattern, index) => (
              <button key={pattern.group} onClick={() => setActive(index)} className={`grid w-full gap-3 border-b border-border py-7 text-left transition-all focus-visible:outline-2 focus-visible:outline-ring md:grid-cols-[150px_1fr_auto] ${index === active ? "pl-4" : "text-muted-foreground hover:text-foreground"}`}>
                <span className="font-mono text-[9px] uppercase tracking-[0.16em]">{pattern.group}</span><span className="font-display text-3xl">{pattern.title}</span><ArrowUpRight className="size-4" />
              </button>
            ))}
          </div>
          <aside className="relative min-h-[440px] overflow-hidden bg-field p-7 md:p-10">
            <div className="absolute inset-x-0 top-1/2 border-t border-dashed border-border"/><div className="absolute bottom-8 left-8 top-8 border-l border-dashed border-border"/>
            <div className="relative flex h-full flex-col justify-between"><div><div className="font-mono text-[9px] uppercase tracking-[0.18em] text-muted-foreground">Closer look · 0{active + 1}</div><h2 className="mt-8 font-display text-5xl">{selectedPattern.stat}</h2><p className="mt-5 max-w-sm text-sm leading-6 text-muted-foreground">{selectedPattern.body}</p></div><div className="flex items-end gap-2" aria-hidden="true">{[28,54,39,76,48,91,63,34,70].map((height, i) => <span key={i} className={i === active + 4 ? "w-3 bg-coral" : "w-3 bg-sage"} style={{ height }} />)}</div></div>
          </aside>
        </div>
      </section>
    </LifeTraceShell>
