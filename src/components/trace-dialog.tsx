...
export function TraceDialog({ moment, moments, open, onOpenChange, onMomentChange }: { moment: LifeMoment | null; moments: LifeMoment[]; open: boolean; onOpenChange: (open: boolean) => void; onMomentChange?: (moment: LifeMoment) => void }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto border-border bg-background p-6 shadow-paper sm:p-10">
    <DialogHeader><div className="mb-4 font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Follow a moment</div><DialogTitle className="font-display text-4xl font-normal md:text-5xl">{moment.title}</DialogTitle><DialogDescription className="pt-2 text-sm">{new Date(moment.occurredAt).toLocaleString()} · {moment.subtitle}</DialogDescription></DialogHeader>
    <p className="border-l border-border pl-4 text-sm leading-6 text-muted-foreground">Connections below reflect time proximity only. They suggest adjacency, never cause.</p>
    <div className="mt-6 overflow-x-auto pb-3"><div className="flex min-w-max items-center gap-3">
      {[moment, ...connected].map((item, index) => <div key={item.id} className="flex items-center gap-3">
        ...
        <Button variant="artifact" onClick={() => onMomentChange?.(item)} className="h-auto w-36 flex-col items-start whitespace-normal p-4 text-left">
